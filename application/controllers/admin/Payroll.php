...
class Payroll extends MY_Controller {
	...
	public function add_pay_monthly() {
		...
		'total_allowances' => $this->input->post('allowance_amount'),
		...
		'PT' => $this->input->post('pt_month'),
		'PF' => $this->input->post('pf_de'),
		'ESIC' => $this->input->post('esic'),
		'TDS' => $this->input->post('tds'),
		'HRA' => $this->input->post('HRA'),
		'Conv_Allow' => $this->input->post('Conv_Allow'),
		'Other_Allow' => $this->input->post('Other_Allow'),
		'LWW' => $this->input->post('LWW'),
		'grand_net_salary'=>$this->input->post('payment_amount'),
	 ...
	 public function pdf_create(){
		...
		$tblbrk = '<table cellpadding="3" cellspacing="0" border="1"><tr bgcolor="#69e48a">
			...
			//allowances
					  ...
					  $tblbrk .= '<tr>
					<td colspan="2">HRA</td>
					<td align="center">'.$this->Xin_model->currency_sign($payment[0]->HRA).'</td>	
					<td>&nbsp;</td>				
					</tr>
					<tr>
					<td colspan="2">Conv_Allow</td>
					<td align="center">'.$this->Xin_model->currency_sign($payment[0]->Conv_Allow).'</td>	
					<td>&nbsp;</td>				
					</tr>
					<tr>
					<td colspan="2">Other_Allow</td>
					<td align="center">'.$this->Xin_model->currency_sign($payment[0]->Other_Allow).'</td>	
					<td>&nbsp;</td>				
					</tr>
					<tr>
					<td colspan="2">LWW</td>
					<td align="center">'.$this->Xin_model->currency_sign($payment[0]->LWW).'</td>	
					<td>&nbsp;</td>				
					</tr>';
			...
			//statutory_deductions
				...
				$tblbrk .= '<tr>
					<td colspan="2">PT</td>
					<td>&nbsp;</td>
					<td align="center">'.$this->Xin_model->currency_sign($payment[0]->PT).'</td>			
					</tr>
					<tr>
					<td colspan="2">PF</td>
					<td>&nbsp;</td>
					<td align="center">'.$this->Xin_model->currency_sign($payment[0]->PF).'</td>			
					</tr>
					<tr>
					<td colspan="2">ESIC</td>
					<td>&nbsp;</td>
					<td align="center">'.$this->Xin_model->currency_sign($payment[0]->ESIC).'</td>			
					</tr>
					<tr>
					<td colspan="2">TDS</td>
					<td>&nbsp;</td>
					<td align="center">'.$this->Xin_model->currency_sign($payment[0]->TDS).'</td>			
					</tr>';
			...
			<tr><td colspan="2" align="center"><strong>Total</strong></td>
				...
				$total_earning = $bs + (float)$payment[0]->HRA + (float)$payment[0]->Conv_Allow + (float)$payment[0]->Other_Allow + (float)$payment[0]->LWW  + $overtime_amount + $commissions_amount + $other_payments_amount;
				$total_deduction = (float)$payment[0]->PT + (float)$payment[0]->PF + (float)$payment[0]->ESIC  + (float)$payment[0]->TDS;
				$total_net_salary = $total_earning - $total_deduction;
				$tblbrk .= '
			<tr><td colspan="2" align="center"><strong>Total</strong></td>
		...
		// 	<tr>
		// 		<td align="right" colspan="1">HRA</td>
		// 		<td align="right" colspan="1">'.$payment[0]->HRA.'</td>

		// 	</tr>
		// </table>';
		// $pdf->writeHTML($tblAll_Ded, true, false, false, false, '');
