    ...
    public function interview_list()
     {
        ...
        foreach($interview->result() as $r) {
			if(in_array('388',$role_resources_ids)) {
				...
				// user full name
				if(!is_null($u_added)){
					$int_addedby = $u_added[0]->first_name. ' '.$u_added[0]->middle_name. ' '.$u_added[0]->last_name;
				} else {
					$int_addedby = '--';	
				}
				// interview message
				...
				$interviewees = '<ol class="nl">';
					...
					$user_intwee = $this->Xin_model->read_user_info($interviewees_id);
						if(!is_null($user_intwee)){
							$interviewees .= '<li>'.$user_intwee[0]->first_name. ' '.$user_intwee[0]->middle_name. ' '.$user_intwee[0]->last_name.'</li>';
						} else {
							$interviewees .= '';	
						}
					}
				...
				foreach(explode(',',$r->interviewers_id) as $interviewers_id) {
					$user_intwer = $this->Xin_model->read_user_info($interviewers_id);
						if(!is_null($user_intwer)){
							$interviewers .= '<li>'.$user_intwer[0]->first_name. ' '.$user_intwer[0]->middle_name. ' '.$user_intwer[0]->last_name.'</li>';
						} else {
							$interviewers .= '';	
						}
					}
