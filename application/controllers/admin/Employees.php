...
class Employees extends MY_Controller
{
	...
	public function employees_list()
	{
		...
		foreach ($employee->result() as $r) {
			...
			elseif ($r->is_active == 2) : $status_btn = 'btn-outline-danger';
				$status_title = $this->lang->line('xin_employees_inactive');
			elseif ($r->is_active == 3) : $status_btn = 'btn btn-primary';
				$status_title = $this->lang->line('xin_employees_offersend');
			elseif ($r->is_active == 4) : $status_btn = 'btn btn-warning';
				$status_title = $this->lang->line('xin_employees_offeraccepted');
			elseif ($r->is_active == 5) : $status_btn = 'btn btn-primary';
				$status_title = $this->lang->line('xin_employees_appointment');
			endif;

			$role_status = $role_name . '<br><div class="btn-group" data-toggle="tooltip" data-state="primary" data-placement="top" title="' . $this->lang->line('xin_change_status') . '"><button type="button" class="btn btn-sm md-btn-flat dropdown-toggle ' . $status_btn . '" data-toggle="dropdown">' . $status_title . '</button><div class="dropdown-menu">
			...
			<a class="dropdown-item statusinfo" href="javascript:void(0)" data-status="2" data-user-id="' . $r->user_id . '">' . $this->lang->line('xin_employees_inactive') . '</a>
			<a class="dropdown-item statusinfo" href="javascript:void(0)" data-status="3" data-user-id="' . $r->user_id . '">' . $this->lang->line('xin_employees_offersend') . '</a>
			<a class="dropdown-item statusinfo" href="javascript:void(0)" data-status="4" data-user-id="' . $r->user_id . '">' . $this->lang->line('xin_employees_offeraccepted') . '</a>
			<a class="dropdown-item statusinfo" href="javascript:void(0)" data-status="5" data-user-id="' . $r->user_id . '">' . $this->lang->line('xin_employees_appointment') . '</a>
			...
			// send mail
			$sendoff = '<span data-toggle="tooltip" data-placement="top" title="Send Offer Letter"><button type="button" class="btn btn-xs btn-primary"> <span class="ion ion-md-mail"></span></button></span>';
			$appointment = '<span data-toggle="tooltip" data-placement="top" title="Send Appointmanet Letter"><button type="button" class="btn btn-xs btn-primary"> <span class="ion ion-md-mail"></span> </button></span>';
			$sendbutton = $sendoff . $appointment;
...
<p>
	...
	{
		$id = $this->Employees_model->lastId();
		...
		if ($this->input->post('add_type') == 'employee') {
			...
			if ($this->input->post('first_name') === '') {
				...
				// 	}*/ else if($this->input->post('middle_name')==='') {
				// 	$Return['error'] = $this->lang->line('xin_employee_error_middle_name');
			...
			$location_id_p = $this->input->post('company_id');
			$designation = $this->input->post('designation_id');
			$result1 = $this->Department_model->ajax_get_comapny_allowances($location_id_p, $designation);
			var_dump($result1);
			foreach ($result1 as $allowance) {
				$allowance_data = array(
					'allowance_title' => $allowance->allowance_title,
					'allowance_amount' => $allowance->allowance_amount,
					'employee_id' => $new_id,
					'is_allowance_taxable' => $allowance->is_allowance_taxable,
					'amount_option' => $allowance->amount_option,
					'company_allowance_id' => $allowance->allowance_id,
					'pf_option' => $allowance->pf_option
				);
				$this->Employees_model->add_salary_allowances($allowance_data);
			}
			// $result2 = $this->Department_model->ajax_get_company_deduction($location_id_p);
			// foreach ($result2 as $deduction) {
			// 	$deduction_data = array(
			// 		'deduction_title' => $deduction->deduction_title,
			// 		'deduction_amount' => $deduction->deduction_amount,
			// 		'statutory_options' => $deduction->statutory_options,
			// 		'employee_id' =>$new_id
			// 		);
			// 		$this->Employees_model->add_salary_statutory_deductions($deduction_data);
			// }
			$data = array(
				...
				'wages_type' => $this->input->post('salary_select'),
				'created_at' => date('Y-m-d h:i:s'),
				'pf_allow' => $this->input->post('pf_allow'),
	...
	public function get_salary($id)
	{
		...
		// $res=$this->Designation_model->ajax_is_salary_information($id);
		// var_dump($res);
		// echo (json_encode($res));
		$this->load->view('admin/employees/get_salary', ["id" => $id]);
	}
	...
	public function transfer_info()
	{
		...
		$data = array(
			...
			'location_id' => $this->input->post('location_id1'),
			'department_id' => $this->input->post('department_id1'),
			'sub_department_id' => $this->input->post('subdepartment_id1'),
			'designation_id' => $this->input->post('designation_id1')
		);
		$location_id_p = $this->input->post('company_id');
		$designation = $this->input->post('designation_id1');
		$result1 = $this->Department_model->ajax_get_comapny_allowances($location_id_p, $designation);
		var_dump($result1);
		foreach ($result1 as $allowance) {
			$allowance_data = array(
			...
			$this->Employees_model->update_salary_allowances($allowance_data, $this->input->post('user_id'));
		}
		$result2 = $this->Designation_model->ajax_is_salary_information($this->input->post('designation_id1'));
		foreach ($result2 as $salary) {
			$basic_salary = array(
				'basic_salary' => $salary->basic_salary
			);
		}
		$result = $this->Employees_model->update_record($data, $this->input->post('user_id'));
		$salary_update = $this->Employees_model->update_record($basic_salary, $this->input->post('user_id'));
	...
	// Update Salary With Allowances
	public function UpdateSalary(){
		
		 $id= $this->input->post('user_id');
		 $salary = $this->input->post('basic_salary');
	 	$data = $this->input->post('update_allowances');
	 	$results = $this->Employees_model->update_allowances($data[0],$data[1],$data[2],$data[3],$data[4],$id,$salary);

		$date= date('Y-m-d', strtotime('last day of last month'));
        $effectiveDate = $this->input->post('month_year');
		
	 $presentDays = $this->Employees_model->total_days($id,$effectiveDate,$date);
	 $details = $this->Employees_model->forEffective_sallary_emp_detail($id);
		...
		foreach ($esic as $result) {
		  ...
		  $edli=0;
		  $admin_charge=0;
		   $check = $this->Timesheet_model->checkLeave($id,$effectiveDate,$date);
		  foreach($check as $leave){
			$leave_days = $leave->leave_days;
			$days = $leave->Days;
		  }
		  $total_leaves=$leave_days+$days;
		  $present_days=$present+$total_leaves;

		  $fraction = $present_days / 26;
		  $basic_salary = $salary * $fraction;
		  $allowances = $this->Employees_model->set_employee_allowances($id);
		 ...
		 $pf_option = $this->Employees_model->Pf_allow($id);
		 foreach($pf_option as $pf){
			 $allow = $pf->pf_allow;
		 }
		...
		$previous_salary = $this->Employees_model->last_salary($id,$effectiveDate,$date);
		$previous_month_salary = 0;
		foreach($previous_salary as $salary){
			$previous_month_salary+=$salary->grand_net_salary;
		}		
