...
class Xin_model extends CI_Model {
	...
	public function employee_leave_date_calendar($current_date) {
		...
		$this->db->query("SET SESSION sql_mode = ''");
		$sql = "SELECT la.*,e.user_id,e.first_name,e.middle_name,e.last_name from xin_leave_applications as la, xin_employees as e where (la.from_date between la.from_date and la.to_date) or la.from_date = ? and la.to_date = ? and e.user_id=la.employee_id and la.status=2 group by la.employee_id";
	...
	public function employees_upcoming_birthday() {
		...
		$query = $this->db->query("SELECT `user_id`, `first_name`, `middle_name`, `last_name`, `date_of_birth`,
    DATE_ADD(
        date_of_birth, 
...
ORDER BY `expiry_date`");
	...
	// total leaves W.R.T to current date
	public function total_leaves($date) {
	
		$sql = "SELECT count(*) as leave_number from xin_leave_applications where from_date= ? AND status=2 or ? BETWEEN from_date AND to_date";
		$binds = array($date,$date);
		$query = $this->db->query($sql,$binds);
		$res = $query->result();
		return $res;
	}
