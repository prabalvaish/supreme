...
class Employees_model extends CI_Model {
   ...
   // get employee allowances
	...
	public function set_employee_allowances_payslip($id) {
		...
		$sql = 'SELECT DA,HRA,Conv_Allow,Other_Allow,LWW FROM xin_salary_payslips WHERE payslip_id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
	...
	public function set_employee_statutory_deductions_payslip($id) {
		...
		$sql = 'SELECT PT,PF,ESIC,TDS,LWF FROM xin_salary_payslips WHERE payslip_id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
	...
	// Read Pf Allow
	public function read_pf_allow($id) {
	
		$sql = 'SELECT pf_allow,tds_type FROM xin_employees WHERE user_id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
...
// Function to add record in table > allowance
public function  update_salary_allowances($data,$id){
...
public function lastId(){
	$sql = 'SELECT (MAX(user_id)+1) id FROM `xin_employees` ';
		$binds = array();
		$query = $this->db->query($sql, $binds);
	...
	// get singledesignation
	public function getDesignation($id){
		$sql = 'SELECT designation_id FROM xin_employees WHERE user_id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
	...
	// Get Designation wise overtime amount
	public function getOvertimeAmount($id) {
		$sql = 'SELECT overtime_rate FROM xin_comapny_overtime WHERE designation_id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
	...
	public function sallary_emp_detail($id) {
		$sql = 'select xin_employees.gender,xin_employees.tds_type,xin_employees.basic_salary,xin_employees.location_id, xin_departments.department_name,xin_employees.pf_allow,xin_designations.designation_name from xin_employees inner join xin_departments on xin_departments.department_id=xin_employees.department_id inner join xin_designations on xin_employees.designation_id= xin_designations.designation_id where xin_employees.employee_id=?';
	...
	public function sallary_emp_details($id) {
		$sql = 'select xin_employees.gender,xin_employees.tds_type,xin_employees.basic_salary,xin_employees.location_id, xin_departments.department_name,xin_employees.pf_allow,xin_designations.designation_name from xin_employees inner join xin_departments on xin_departments.department_id=xin_employees.department_id inner join xin_designations on xin_employees.designation_id= xin_designations.designation_id where xin_employees.user_id=?';
...
// Total Days
public function total_days($id,$start_date,$end_date) {
	$sql = 'SELECT COUNT(attendance_status) AS Total from xin_attendance_time where employee_id=? AND DATE(attendance_date) BETWEEN ? AND ?';
...
public function userid($id) {
	$sql = 'SELECT user_id,pf_allow FROM `xin_employees` WHERE employee_id=?';
...
public function Pf_allow($id) {
	$sql = 'SELECT pf_allow FROM `xin_employees` WHERE user_id=?';
...
public function read_info($id) {
	...
	$sql = 'select xin_employees.company_id,xin_office_location.location_name, xin_departments.department_name,xin_designations.designation_name,xin_companies.name, xin_sub_departments.department_name from xin_employees inner join xin_departments on xin_departments.department_id=xin_employees.department_id inner join xin_designations on xin_employees.designation_id= xin_designations.designation_id inner join xin_office_location on xin_employees.location_id=xin_office_location.location_id inner join xin_companies on xin_employees.company_id=xin_companies.company_id inner join xin_sub_departments on xin_sub_departments.sub_department_id= xin_employees.sub_department_id where xin_employees.user_id=?';
	$binds = array($id);
	$query = $this->db->query($sql, $binds);
	return $query->result();
}
// Update Transfer Info
public function transfer_info1($data,$id) {
...
// Salary Type of Customer
public function salary_type($id) {
	
	$sql = 'SELECT salary_type FROM xin_office_location WHERE location_id=?';
	$binds = array($id);
	$query = $this->db->query($sql, $binds);
	return $query->result();
}
// Emp Details W.R.T. user_id
public function forEffective_sallary_emp_detail($id) {
	$sql = 'select xin_employees.gender,xin_employees.tds_type,xin_employees.basic_salary,xin_employees.location_id, xin_departments.department_name,xin_employees.pf_allow,xin_designations.designation_name from xin_employees inner join xin_departments on xin_departments.department_id=xin_employees.department_id inner join xin_designations on xin_employees.designation_id= xin_designations.designation_id where xin_employees.user_id=?';
  ...
  public function update_allowances($DA,$HRA,$con_allo,$other_allow,$LWW,$id,$salary){
	  $sql ="UPDATE xin_salary_allowances SET allowance_amount = ? WHERE allowance_title = 'DA' AND employee_id=?";
	  $bind = array($DA,$id);
	  $query = $this->db->query($sql,$bind);
	  $sql1 ="UPDATE xin_salary_allowances SET allowance_amount = ? WHERE allowance_title = 'HRA' AND employee_id=?";
	  $bind1 = array($HRA,$id);
	  $query = $this->db->query($sql1,$bind1);
	  $sql2 ="UPDATE xin_salary_allowances SET allowance_amount = ? WHERE allowance_title = 'Conv. Allow' AND employee_id=?";
	  $bind2 = array($con_allo,$id);
	  $query = $this->db->query($sql2,$bind2);
	  $sql3 ="UPDATE xin_salary_allowances SET allowance_amount = ? WHERE allowance_title = 'Other Allow' AND employee_id=?";
	  $bind3 = array($other_allow,$id);
	  $query = $this->db->query($sql3,$bind3);
	  $sql4 ="UPDATE xin_salary_allowances SET allowance_amount = ? WHERE allowance_title = 'LWW' AND employee_id=?";
	  $bind4 = array($LWW,$id);
	  $query = $this->db->query($sql4,$bind4);
	  $sql5 ="UPDATE xin_employees SET basic_salary = ? WHERE  user_id=?";
	  $bind5 = array($salary,$id);
	  $query = $this->db->query($sql5,$bind5);
  }
//   salary with respect to effective date
public function last_salary($id,$effectiveDate,$date){
	$sql = "SELECT grand_net_salary from xin_salary_payslips where employee_id=? AND salary_month BETWEEN ? AND ? ";
	$binds=array($id,$effectiveDate,$date);
	$query = $this->db->query($sql,$binds);
	return $query->result();
}
// Update Arrear Amount
public function update_arrear_amount($id,$arrear_amount){
