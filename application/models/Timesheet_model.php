<?php
	...
	// check leave
	public function checkLeave($employee_id,$month_start,$month_end) {
	
		$sql = 'SELECT count(*) as leave_days, DATEDIFF(to_date, from_date) as Days from xin_leave_applications WHERE employee_id =? AND status =2 AND DATE(applied_on) BETWEEN ? AND ?';
		$binds = array($employee_id,$month_start,$month_end);
		$query = $this->db->query($sql, $binds);
		return $query->result();
	}
